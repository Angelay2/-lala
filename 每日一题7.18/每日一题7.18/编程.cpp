/*
1. 解读密码
nowcoder要和朋友交流一些敏感的信息，例如他的电话号码等。因此他要对这些敏感信息进行混淆，比如在 数字中间掺入一些额外的符号，让它看起来像一堆乱码。 现在请你帮忙开发一款程序，解析从nowcoder那儿接收到的信息，读取出中间有用的信息。 
...
/*
2. 走迷宫
NowCoder最喜欢游乐场的迷宫游戏，他和小伙伴们比赛谁先走出迷宫。 现在把迷宫的地图给你，你能帮他算出最快走出迷宫需要多少步吗？ 输入描述： 输入包含多组数据。 每组数据包含一个10*10，由“#”和“.”组成的迷宫。其中“#”代表墙；“.”代表通路。 入口在第一行第二列；出口在最后一行第九列。 从任意一个“.”点都能一步走到上下左右四个方向的“.”点。 输出描述： 对应每组数据，输出从入口到出口最短需要几步。

【题目解析】
采用二维数组保存地图数据，如下图所示，'#'代表墙，' . ' 代表通路，在每个路口时，可以向上下左右4个方
向移动(除过边界)，当一条路径不通时，需要回溯到上一步，回溯到上一个路口后再向其他方向探索，直到走
到出口，一条路径找到，求解出步数，再回退找其他路径，求解出最短的路径。以上方式是采用深度优先方式遍历，但是深度优先方式找到一条路径后，该路径不一定是最短路径，需要将所有路径找到后比较，才能确定出最终的最短路径。
