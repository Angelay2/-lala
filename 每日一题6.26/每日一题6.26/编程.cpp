...
public: 
	int getValue(vector<int> gifts, int n) {
		...
		//超过一半的数排序之后必然排在中间, 找出总数大于一半的那个金额 
		int middle = gifts[n / 2]; 
		int count = 0;
		for(int i = 0; i < n; i++) {
			//统计排在中间的数的个数 统计红包金额出现的次数
			if(gifts[i] == middle) {
				count++;
			}
		}
...
public: 
	int getValue(vector<int> gifts, int n) {
		map<int,int> count;
		int middle = gifts.size() / 2; // 一半有多少个
		for(const auto& e : gifts) {
			++count[e];
		}
		for(const auto& e : count) { 
			if(e.second >= middle) // 个数>=一半
				return e.first; // 返回金额
		}
		return 0;
	}
...
/*
2. 【计算字符串的距离】 Levenshtein 距离，又称编辑距离，指的是两个字符串之间，由一个转换成另一个所需的最少编辑操作次数。许可的编辑操作包括将一个字符替换成另一个字符，插入一个字符，删除一个字符。编辑距离的算法是首先由俄国科学家Levenshtein提出的，故又叫Levenshtein Distance。 
Ex： 
字符串A: abcdefg 
字符串B: abcdef 
