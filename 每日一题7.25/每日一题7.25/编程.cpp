/*
1. 顺时针打印矩阵 
语言限制：[Python, C++, C#, Java] 对于一个矩阵，请设计一个算法从左上角(mat[0][0])开始，顺时针打印矩阵元素。
...
【解题思路】：
1. 按照题意, 将i为0到n-1的情况都罗列出来即可. 2. 创建 mxL 记录i位左侧最大值, 创建 mxR 记录i位右侧最大值. 这是在找最大值. 3. 核心要体会到 第 i 位左侧的最大值就是 i - 1 位最大值和 i 的值的较大值.
...
/*
2. ：左右最值最大差
语言限制：[Python, C++, C#, Java] 给定一个长度为N(N>1)的整型数组A，可以将A划分成左右两个部分，左部分A[0..K]，右部分A[K+1..N-1]，K 可以取值的范围是[0,N-2]。求这么多划分方案中，左部分中的最大值减去右部分最大值的绝对值，最大是多 少？给定整数数组A和数组的大小n，请返回题目所求的答案。

【解题思路】：
1. 先记录左上角和右下角坐标(这两个坐标就描述了一个矩形) 2. 然后先按照顺时针打印这个矩形边上的元素
3. 缩小矩形(也就是调整左上和右下坐标位置) 4. 再次顺时针打印. 一直缩小到这个矩形为空即可.
*/
// 问题本身不复杂, 将思路考虑周全即可.
class Printer { 
public: 
